...
def _get_dataset_class():
    """Загружает Dataset только для сценариев, которым нужен RAGAS dataset."""
    from datasets import Dataset
    return Dataset


def _get_ragas_components():
    """Ленивая загрузка RAGAS, чтобы быстрые тесты не требовали optional integrations."""
...
def run_ragas_evaluation(pipeline):
    ...
    evaluate, faithfulness, context_precision = _get_ragas_components()
