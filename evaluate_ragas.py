...
def _get_ragas_components():
    """Импортирует RAGAS только при запуске оценки, чтобы helpers были доступны без optional интеграций."""
...
def run_ragas_evaluation(pipeline):
    ...
    ragas_evaluate, faithfulness, context_precision = _get_ragas_components()
    metrics_to_use = [faithfulness(), context_precision()]
    try:
        result = ragas_evaluate(dataset=dataset, metrics=metrics_to_use)
