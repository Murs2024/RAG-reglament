...
def _get_ragas_components():
    """Lazy-load RAGAS so non-RAGAS helpers remain importable in lightweight tests."""
...
def run_ragas_evaluation(pipeline):
    ...
    evaluate_fn, faithfulness, context_precision = _get_ragas_components()
    metrics_to_use = [faithfulness(), context_precision()]
    try:
        result = evaluate_fn(dataset=dataset, metrics=metrics_to_use)
